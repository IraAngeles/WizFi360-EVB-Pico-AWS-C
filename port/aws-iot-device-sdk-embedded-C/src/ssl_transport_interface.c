...
int ssl_transport_init(tlsContext_t *tlsContext, int *socket_fd, const char *host)
{
    ...
    */
    ...
    mbedtls_ssl_set_bio(&tlsContext->ssl, socket_fd, iotSocketSend, NULL, recv_timeout);
    //mbedtls_ssl_set_bio(&tlsContext->ssl, socket_fd, iotSocketSend, iotSocketRecv, NULL);
...
int ssl_socket_connect(tlsContext_t *tlsContext, const char *host, char *addr, unsigned int port)
{
    int ret;
    int32_t af;
    uint32_t ip_len = 4;
    ...
    ret = iotSocketGetHostByName (host, af, addr, &ip_len);
    if (ret < 0)
        return (-1);
    ...
    if (iotSocketConnect (sock, addr, 4, port) < 0) {
        iotSocketClose (sock);
        return (-1);
    }
    tlsContext->fd = sock;
